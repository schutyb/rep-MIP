...
for i in range(len(arr_g_psrn)):
    plt.plot(filt_time, arr_g_psrn[i], '-x')
plt.title('PSNR(filtering times) for G')
plt.xlabel('Times (0 ; 5)')
plt.ylabel('PSNR[dB]')
plt.grid()

plt.figure(2)
for i in range(len(arr_s_psrn)):
    plt.plot(arr_s_psrn[i], '-x')
plt.title('PSNR(filtering times) for S')
plt.xlabel('Times (0 ; 5)')
plt.ylabel('PSNR[dB]')
plt.grid()
