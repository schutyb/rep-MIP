...
one = False
if one:
    filename = '/home/bruno/Documentos/Proyectos/TESIS/TESIS/Data/ometiff/16952_SP_mel_bo.ome.tiff'
    im = tifffile.imread(filename)
    im2 = tifffile.imread(
        '/home/bruno/Documentos/Proyectos/TESIS/TESIS/Data/melanocitos/lsm/channel/16952_channel_mel_borde.lsm')
    ...
    for i in range(len(hist)):
        if hist[i] > int(0.01 * max(hist)):  # sacos las fases que en el histograma tiene menos del 1% del máximo
            # del hist
            fases.append(bins[i])
    ...
    for i in range(aux.shape[0]):
        for j in range(aux.shape[1]):
            if aux[i][j] == 0:
                ...
                if aux2[i][j] == 0:
                    rgb[i][j][:] = colorsys.hsv_to_rgb((aux[i][j] - (min_ph - 1)) * (1 / (n * abs(max_ph - min_ph))), 0,
                                                       1)
                else:
        ...
        for i in range(aux.shape[0]):
            for j in range(aux.shape[1]):
                if aux[i][j] == 0:
                    ...
                    rgb[i][j][:] = colorsys.hsv_to_rgb((aux[i][j] - (min_ph - 1)) * (1 / (n * abs(max_ph - min_ph))), 1,
                                                       1)

        muestra = files_names[k][0:5]
        plt.imsave('/home/bruno/Documentos/Proyectos/TESIS/TESIS/Data/pseudocolor/' + muestra + '.png', rgb)
